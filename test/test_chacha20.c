...
static int test_chacha20_rfc8439_vector(void)
{
    ...
    unsigned char plaintext[] = "Ladies and Gentlemen of the class of '99: "
    ...
    ASSERT_EQUAL(chacha20_process(&ctx, plaintext, output, sizeof(plaintext) - 1), 0,
...
static int test_chacha20_symmetry(void)
{
    ...
    ASSERT_EQUAL(chacha20_process(&ctx, plaintext, ciphertext, sizeof(plaintext)), 0,
                 "Encryption failed");
    chacha20_cleanup(&ctx);

    /* Decrypt */
    ASSERT_EQUAL(chacha20_init(&ctx, key, nonce, 0), 0, "Decrypt init failed");
    ASSERT_EQUAL(chacha20_process(&ctx, ciphertext, decrypted, sizeof(plaintext)), 0,
                 "Decryption failed");
    chacha20_cleanup(&ctx);
