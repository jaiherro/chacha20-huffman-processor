...
static int create_test_file(const char *filename, const unsigned char *content, size_t size)
{
...
static int read_test_file(const char *filename, unsigned char *buffer, size_t buffer_size, size_t *bytes_read)
{
...
static int test_huffman_basic_symmetry(void)
{
    ...
    const unsigned char test_data[] = "The quick brown fox jumps over the lazy dog. This is a test of Huffman compression with repeated characters and words.";
    ...
    /* Compress the file */
    ASSERT_EQUAL(huffman_compress_file(TEST_INPUT_FILE, TEST_COMPRESSED_FILE, 1), 0,
                 "Huffman compression failed");

    /* Decompress the file */
    ASSERT_EQUAL(huffman_stream_decompress_file(TEST_COMPRESSED_FILE, TEST_OUTPUT_FILE, 1), 0,
                 "Huffman decompression failed");

    /* Read the output and compare */
    ASSERT_EQUAL(read_test_file(TEST_OUTPUT_FILE, output_buffer, sizeof(output_buffer), &output_size), 0,
                 "Failed to read decompressed file");

    ASSERT_EQUAL(output_size, test_size, "Decompressed size doesn't match original");
    ASSERT_MEM_EQUAL(output_buffer, test_data, test_size, "Decompressed content doesn't match original");
...
static int test_huffman_empty_file(void)
{
    ...
    ASSERT_EQUAL(huffman_stream_decompress_file(TEST_COMPRESSED_FILE, TEST_OUTPUT_FILE, 1), 0,
                 "Failed to decompress empty file");
...
static int test_huffman_single_character(void)
{
    ...
    ASSERT_EQUAL(huffman_stream_decompress_file(TEST_COMPRESSED_FILE, TEST_OUTPUT_FILE, 1), 0,
                 "Single character decompression failed");

    ASSERT_EQUAL(read_test_file(TEST_OUTPUT_FILE, output_buffer, sizeof(output_buffer), &output_size), 0,
                 "Failed to read single character output");

    ASSERT_EQUAL(output_size, test_size, "Single character output size mismatch");
    ASSERT_MEM_EQUAL(output_buffer, test_data, test_size, "Single character content mismatch");
...
static int test_huffman_repetitive_data(void)
{
    ...
    memset(test_data, 'A', sizeof(test_data)); /* All A's should compress very well */
    unsigned char output_buffer[1000];
    size_t output_size;

    cleanup_test_files();

    ASSERT_EQUAL(create_test_file(TEST_INPUT_FILE, test_data, sizeof(test_data)), 0,
    ...
    ASSERT_TRUE(compressed_size < original_size / 2, "Repetitive data should compress significantly");

    ASSERT_EQUAL(huffman_stream_decompress_file(TEST_COMPRESSED_FILE, TEST_OUTPUT_FILE, 1), 0,
                 "Repetitive data decompression failed");

    ASSERT_EQUAL(read_test_file(TEST_OUTPUT_FILE, output_buffer, sizeof(output_buffer), &output_size), 0,
                 "Failed to read repetitive output");

    ASSERT_EQUAL(output_size, sizeof(test_data), "Repetitive output size mismatch");
    ASSERT_MEM_EQUAL(output_buffer, test_data, sizeof(test_data), "Repetitive content mismatch");
...
static int test_huffman_diverse_data(void)
{
    ...
    ASSERT_EQUAL(huffman_stream_decompress_file(TEST_COMPRESSED_FILE, TEST_OUTPUT_FILE, 1), 0,
                 "Diverse data decompression failed");

    ASSERT_EQUAL(read_test_file(TEST_OUTPUT_FILE, output_buffer, sizeof(output_buffer), &output_size), 0,
                 "Failed to read diverse output");

    ASSERT_EQUAL(output_size, sizeof(test_data), "Diverse output size mismatch");
    ASSERT_MEM_EQUAL(output_buffer, test_data, sizeof(test_data), "Diverse content mismatch");
...
static int test_huffman_streaming_context(void)
{
    ...
    ASSERT_EQUAL(ctx.input_size, test_size, "Input size not recorded correctly");
    ...
    /* Test compression */
    ASSERT_EQUAL(huffman_stream_compress_file(&ctx, TEST_INPUT_FILE, TEST_COMPRESSED_FILE, 1), 0,
    ...
    ASSERT_EQUAL(huffman_stream_decompress_file(TEST_COMPRESSED_FILE, TEST_OUTPUT_FILE, 1), 0,
                 "Decompression after context cleanup failed");
...
static int test_huffman_error_conditions(void)
{
    ...
    ASSERT_TRUE(huffman_compress_file(NULL, "output", 1) != 0, "Should reject NULL input file");
    ASSERT_TRUE(huffman_compress_file("input", NULL, 1) != 0, "Should reject NULL output file");
    ASSERT_TRUE(huffman_stream_decompress_file(NULL, "output", 1) != 0, "Should reject NULL input for decompression");
    ASSERT_TRUE(huffman_stream_decompress_file("input", NULL, 1) != 0, "Should reject NULL output for decompression");

    /* Test operations on non-existent files */
    cleanup_test_files();
    ASSERT_TRUE(huffman_compress_file("nonexistent_file.txt", TEST_COMPRESSED_FILE, 1) != 0,
                "Should fail on non-existent input file");
    ASSERT_TRUE(huffman_stream_decompress_file("nonexistent_compressed.huf", TEST_OUTPUT_FILE, 1) != 0,
                "Should fail on non-existent compressed file");

    /* Test context operations in wrong order */
    huffman_stream_init(&ctx);
    ASSERT_TRUE(huffman_stream_prepare_encoding(&ctx) != 0, "Should fail prepare without frequency count");
    ASSERT_TRUE(huffman_stream_compress_file(&ctx, "nonexistent", "output", 1) != 0, "Should fail compress without preparation");
    huffman_stream_cleanup(&ctx);
...
static int test_huffman_worst_case_size(void)
{
    ...
    for (size_t i = 0; i < num_tests; i++)
    {
        ...
        ASSERT_TRUE(worst_case >= test_sizes[i], "Worst case should be at least input size");
        ASSERT_TRUE(worst_case > 0 || test_sizes[i] == 0, "Worst case should be positive for non-zero input");
    }
...
static int test_huffman_binary_data(void)
{
    ...
    for (int i = 0; i < 512; i++)
    {
        test_data[i] = (unsigned char)(i % 17); /* Some repetition but not too much */
    }
    ...
    ASSERT_EQUAL(huffman_stream_decompress_file(TEST_COMPRESSED_FILE, TEST_OUTPUT_FILE, 1), 0,
                 "Binary data decompression failed");

    ASSERT_EQUAL(read_test_file(TEST_OUTPUT_FILE, output_buffer, sizeof(output_buffer), &output_size), 0,
                 "Failed to read binary output");

    ASSERT_EQUAL(output_size, sizeof(test_data), "Binary output size mismatch");
    ASSERT_MEM_EQUAL(output_buffer, test_data, sizeof(test_data), "Binary content mismatch");
