...
static int test_key_derivation_basic(void)
{
    ...
    /* Generate salt */
    ASSERT_EQUAL(generate_salt(salt, sizeof(salt)), 0, "Salt generation failed");

    /* Derive key and nonce */
    ASSERT_EQUAL(derive_key_and_nonce(password, salt, sizeof(salt), 1000,
...
static int test_salt_generation(void)
{
    ...
    ASSERT_EQUAL(generate_salt(salt1, sizeof(salt1)), 0, "Salt1 generation failed");
    ASSERT_EQUAL(generate_salt(salt2, sizeof(salt2)), 0, "Salt2 generation failed");
    ASSERT_EQUAL(generate_salt(salt3, sizeof(salt3)), 0, "Salt3 generation failed");
