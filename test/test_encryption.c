...
static int test_chacha20_rfc8439_vector1(void) {
    ...
    // Initial Counter
    uint32_t counter = 1; // As per RFC 8439

    // Plaintext (114 zeros)
    size_t input_len = 114;
    uint8_t *plaintext = (uint8_t *)calloc(input_len, sizeof(uint8_t));
    ASSERT_NOT_NULL(plaintext, "Failed to allocate plaintext buffer");

    // Expected Ciphertext (first 64 bytes shown in RFC)
    uint8_t expected_ciphertext_prefix[64];
    ...
    uint8_t *ciphertext = (uint8_t *)malloc(input_len);
    ...
    uint8_t *decrypted = (uint8_t *)malloc(input_len);
...
static int test_chacha20_basic(void) {
    ...
    uint8_t nonce[CHACHA20_NONCE_SIZE] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
                                          0x08, 0x09, 0x0a, 0x0b};
    uint32_t counter = 42;

    const char *input_str = "This is a secret message.";
    uint8_t *plaintext = (uint8_t *)input_str;
...
static int test_chacha20_multi_block(void) {
     ...
     size_t input_len = CHACHA20_BLOCK_SIZE * 3 + 10; // More than 3 blocks
     uint8_t *plaintext = (uint8_t *)malloc(input_len);
     ASSERT_NOT_NULL(plaintext, "Failed to allocate plaintext buffer");
