    ...
    if (mode == MODE_COMPRESS)
        {
            ...
            processed_size
                = compress_file(input_file, output_file, quiet, &original_size);
    ...
    switch (mode)
        {
            case MODE_ENCRYPT:
                ...
                processed_size = encrypt_file(input_file, output_file, password,
                                              quiet, &original_size);
                DEBUG_INFO("Encryption result - original: %lu bytes, "
                           "encrypted: %lu bytes",
                           original_size, processed_size);
                if (processed_size > 0)
                    {
            ...
            case MODE_DECRYPT:
                ...
                processed_size = decrypt_file(input_file, output_file, password,
                                              quiet, &original_size);
                DEBUG_INFO(
                    "Decryption result - encrypted: %lu bytes, decrypted: "
                    "%lu bytes",
                    original_size, processed_size);
                if (processed_size == 0 && original_size > DEFAULT_SALT_SIZE)
                    {
                        if (!quiet)
                            fprintf(stderr,
                                    "Decryption failed (I/O error or file "
                                    "too small).\n");
                        DEBUG_ERROR_MSG(
            ...
            case MODE_PROCESS:
                ...
                processed_size = process_file(input_file, output_file, password,
                                              quiet, &original_size);
                DEBUG_INFO("Processing result - original: %lu bytes, "
                           "processed: %lu bytes",
                           original_size, processed_size);
                if (processed_size > 0)
                    {
            ...
            case MODE_EXTRACT:
                DEBUG_INFO_MSG(
                    "Starting file extraction (decrypt + decompress)");
                processed_size = extract_file(input_file, output_file, password,
                                              quiet, &original_size);
                DEBUG_INFO(
                    "Extraction result - processed: %lu bytes, extracted: "
                    "%lu bytes",
                    original_size, processed_size);
                /* Treat zero-size decompressed output as valid for empty files
                 * (only header). */
                if (processed_size == 0
                    && original_size > sizeof(unsigned long))
                    {
                        if (!quiet)
                ...
                if (argc < 4)
                    {
                        ...
                        fprintf(stderr,
                                "Usage: %s -b <output_directory> <file1> "
                                "[file2] ...\n",
                                argv[0]);
                        return -1;
                    }
    ...
    switch (operation_mode)
        {
            ...
            case MODE_EXTRACT:
                result
                    = handle_crypto_operation(operation_mode, input_file_arg,
            ...
            case MODE_FIND:
                result
                    = handle_file_list("find", input_file_arg, quiet_operation);
                break;
            case MODE_BATCH:
                {
                    ...
                    result = handle_batch_operation(
                        batch_file_ptrs, num_batch_input_files,
                        batch_output_dir, quiet_operation);
                }
                break;
            ...
            default:
                if (operation_mode != MODE_HELP)
                    {
                        DEBUG_ERROR("Unknown operation mode: %d",
                                    operation_mode);
                    }
                print_usage(argv[0]);
