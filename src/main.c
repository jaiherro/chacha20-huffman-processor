...
static int handle_crypto_operation (int mode, const char *input_file,
                                    const char *output_file, int quiet);
static int handle_batch_operation (char *input_files[], int num_files,
                                   const char *output_dir, int quiet);
static int parse_command_line (int argc, char *argv[], int *mode,
                               char **input_file, char **output_file,
                               char **batch_output_dir,
                               char batch_input_files[][MAX_FILENAME],
                               int *num_batch_files, int *quiet, int *debug);
static int validate_file_input (const char *filename);

/* Helper function implementations */
static int
validate_file_input (const char *filename)
{
    if (!file_exists (filename))
        {
            fprintf (stderr,
                     "ERROR: Input file '%s' does not exist or cannot be "
                     "accessed.\n",
                     filename);
            return -1;
        }
...
static int
handle_compression_operation (int mode, const char *input_file,
                              const char *output_file, int quiet)
{
    unsigned long original_size, processed_size;

    DEBUG_FUNCTION_ENTER ("handle_compression_operation");
    DEBUG_INFO ("Compression operation - mode: %s, input: '%s', output: '%s', "
                "quiet: %s",
                (mode == MODE_COMPRESS) ? "COMPRESS" : "DECOMPRESS", input_file,
                output_file, quiet ? "yes" : "no");
    ...
    if (mode == MODE_COMPRESS)
        {
            ...
            processed_size = compress_file (input_file, output_file, quiet,
                                            &original_size);
            DEBUG_INFO ("Compression result - original: %lu bytes, compressed: "
                        "%lu bytes",
                        original_size, processed_size);

            if (processed_size > 0 || original_size == 0)
                {
                    add_entry_to_file_list (input_file, output_file,
                                            original_size, processed_size,
                                            quiet);
    ...
    else if (mode == MODE_DECOMPRESS)
        {
            ...
            processed_size = decompress_file (input_file, output_file, quiet,
                                              &original_size);
            DEBUG_INFO ("Decompression result - compressed: %lu bytes, "
            ...
            if (processed_size == 0 && original_size > sizeof (unsigned long))
                {
                    if (!quiet)
                        fprintf (stderr, "Decompression failed (corrupted file "
                                         "or I/O error).\n");
                    DEBUG_ERROR (
                        "Decompression failed - invalid result sizes: %s",
                        "error");
                    DEBUG_FUNCTION_EXIT ("handle_compression_operation", 1);
                    return 1;
                }
...
static int
handle_crypto_operation (int mode, const char *input_file,
                         const char *output_file, int quiet)
{
    ...
    const char *mode_names[] = { "",        "COMPRESS", "DECOMPRESS", "ENCRYPT",
                                 "DECRYPT", "PROCESS",  "EXTRACT" };
    DEBUG_INFO (
        "Crypto operation - mode: %s, input: '%s', output: '%s', quiet: %s",
        (mode >= 1 && mode <= 6) ? mode_names[mode] : "UNKNOWN", input_file,
        output_file, quiet ? "yes" : "no");
    ...
    switch (mode)
        ...
        case MODE_ENCRYPT:
            ...
            processed_size = encrypt_file (input_file, output_file, password,
                                           quiet, &original_size);
            DEBUG_INFO (
                "Encryption result - original: %lu bytes, encrypted: %lu bytes",
                original_size, processed_size);
            if (processed_size > 0)
                {
                    add_entry_to_file_list (input_file, output_file,
        ...
        case MODE_DECRYPT:
            ...
            processed_size = decrypt_file (input_file, output_file, password,
                                           quiet, &original_size);
            DEBUG_INFO ("Decryption result - encrypted: %lu bytes, decrypted: "
                        "%lu bytes",
                        original_size, processed_size);
            if (processed_size == 0 && original_size > DEFAULT_SALT_SIZE)
                {
                    if (!quiet)
                        fprintf (stderr, "Decryption failed (I/O error or file "
                                         "too small).\n");
                    DEBUG_ERROR_MSG (
                        "Decryption failed - invalid result sizes");
                    memset (password, 0, sizeof (password));
                    DEBUG_FUNCTION_EXIT ("handle_crypto_operation", 1);
                    return 1;
                }
        ...
        case MODE_PROCESS:
            ...
            processed_size = process_file (input_file, output_file, password,
                                           quiet, &original_size);
            DEBUG_INFO (
                "Processing result - original: %lu bytes, processed: %lu bytes",
                original_size, processed_size);
            if (processed_size > 0)
                {
                    add_entry_to_file_list (input_file, output_file,
        ...
        case MODE_EXTRACT:
            ...
            processed_size = extract_file (input_file, output_file, password,
                                           quiet, &original_size);
            DEBUG_INFO ("Extraction result - processed: %lu bytes, extracted: "
                        "%lu bytes",
                        original_size, processed_size);
            /* Treat zero-size decompressed output as valid for empty files
             * (only header). */
            if (processed_size == 0 && original_size > sizeof (unsigned long))
                {
                    if (!quiet)
                        fprintf (stderr, "Extraction failed (decryption or "
                                         "decompression error).\n");
                    DEBUG_ERROR_MSG (
                        "Extraction failed - invalid result sizes");
                    memset (password, 0, sizeof (password));
                    DEBUG_FUNCTION_EXIT ("handle_crypto_operation", 1);
                    return 1;
                }
...
static int
handle_batch_operation (char *input_files[], int num_files,
                        const char *output_dir, int quiet)
{
    char password[MAX_PASSWORD];
    int result;
    ...
    if (get_password (password, sizeof (password), 1) != 0)
        {
            DEBUG_ERROR ("Failed to get password for batch operation: %s",
                         "failed");
    ...
    result
        = batch_process (input_files, num_files, output_dir, password, quiet);
...
static int
parse_command_line (int argc, char *argv[], int *mode, char **input_file,
                    char **output_file, char **batch_output_dir,
                    char batch_input_files[][MAX_FILENAME],
                    int *num_batch_files, int *quiet, int *debug)
{
    *mode = MODE_HELP;
    ...
    *debug = 0;
    ...
    switch (*mode)
        ...
        case MODE_EXTRACT:
            if (argc < 4)
                {
                    fprintf (stderr,
                             "ERROR: Mode '%s' requires both input and output "
                             "file arguments.\n",
                             argv[1]);
                    fprintf (stderr,
                             "Usage: %s %s <input_file> <output_file>\n",
                             argv[0], argv[1]);
                    return -1;
                }
            *input_file = argv[2];
        ...
        case MODE_FIND:
            if (argc < 3)
                {
                    fprintf (stderr,
                             "ERROR: Find mode requires a search pattern.\n");
        ...
        case MODE_BATCH:
            if (argc < 4)
                {
                    fprintf (stderr,
                             "ERROR: Batch mode requires output directory and "
                             "at least one input file.\n");
                    fprintf (
                        stderr,
                        "Usage: %s -b <output_directory> <file1> [file2] ...\n",
                        argv[0]);
                    return -1;
                }
            ...
            if (strcmp (argv[i], "-q") == 0)
            ...
            else if (*mode == MODE_BATCH)
                {
                    if (*num_batch_files < MAX_BATCH_FILES)
                        {
                            strncpy (batch_input_files[*num_batch_files],
                                     argv[i], MAX_FILENAME - 1);
                            batch_input_files[*num_batch_files]
                                             [MAX_FILENAME - 1]
                                = '\0';
                            (*num_batch_files)++;
                        }
    ...
    if (*mode == MODE_BATCH && *num_batch_files == 0)
        {
            fprintf (stderr,
                     "ERROR: No input files specified for batch processing.\n");
            fprintf (stderr,
                     "Usage: %s -b <output_directory> <file1> [file2] ...\n",
                     argv[0]);
            return -1;
...
main (int argc, char *argv[])
{
    ...
    if (parse_command_line (argc, argv, &operation_mode, &input_file_arg,
                            ...
                            &quiet_operation, &debug_operation)
        != 0)
        {
            print_usage (argv[0]);
            return 1;
    ...
    if (debug_operation)
        {
            ...
            DEBUG_INFO ("Debug mode enabled for Secure File Processor: %s",
                        "enabled");
    ...
    switch (operation_mode)
        ...
        case MODE_COMPRESS:
        case MODE_DECOMPRESS:
            result = handle_compression_operation (
                operation_mode, input_file_arg, output_file_arg,
                quiet_operation);
            break;
        ...
        case MODE_PROCESS:
        case MODE_EXTRACT:
            result = handle_crypto_operation (operation_mode, input_file_arg,
                                              output_file_arg, quiet_operation);
            break;
        ...
        case MODE_BATCH:
            {
                ...
                result = handle_batch_operation (
                    batch_file_ptrs, num_batch_input_files, batch_output_dir,
                    quiet_operation);
            }
            break;
    ...
    DEBUG_INFO ("Program exiting with status: %s",
