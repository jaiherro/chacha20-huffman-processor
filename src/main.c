                                        ...
                                        const char *output_file, int quiet)
{
    ...
    if (mode == MODE_COMPRESS)
    {
        ...
        {
            add_entry_to_file_list(input_file, output_file, original_size,
                                   processed_size, quiet);
    ...
    else if (mode == MODE_DECOMPRESS)
    {
        ...
        processed_size =
            decompress_file(input_file, output_file, quiet, &original_size);
        ...
        if (processed_size == 0 && original_size > sizeof(unsigned long))
        {
            if (!quiet)
                fprintf(stderr, "Decompression failed (corrupted file "
                                "or I/O error).\n");
            DEBUG_ERROR("Decompression failed - invalid result sizes: %s",
                        "error");
            DEBUG_FUNCTION_EXIT("handle_compression_operation", 1);
            return 1;
        }
    ...
    *debug            = 0;
    ...
    for (int i = arg_index; i < argc; i++)
    {
        ...
        else if (*mode == MODE_BATCH)
        {
            ...
            {
                strncpy(batch_input_files[*num_batch_files], argv[i],
                        MAX_FILENAME - 1);
...
int main(int argc, char *argv[])
{
    ...
    switch (operation_mode)
    {
        ...
        case MODE_DECOMPRESS:
            result =
                handle_compression_operation(operation_mode, input_file_arg,
        ...
        case MODE_BATCH:
        {
            ...
            result =
                handle_batch_operation(batch_file_ptrs, num_batch_input_files,
