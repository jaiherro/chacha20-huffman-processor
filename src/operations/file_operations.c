...
static int init_crypto_context(crypto_operation_t *op, const char *password)
{
    ...
    if (derive_key_and_nonce(
            password, op->salt, DEFAULT_SALT_SIZE, KEY_DERIVATION_ITERATIONS,
                                           ...
                                           int encrypt)
{
    ...
    {
        unsigned long chunk_size = (data_size - processed_size < BUFFER_SIZE)
                           ...
                           unsigned long processed_size, int quiet)
{
                      ...
                      processed_size) != 0)
    {
        ...
        DEBUG_ERROR("Failed to add entry to file list in memory: '%s' -> '%s'",
                    input_file, output_file);
        file_list_free(&file_list);
        DEBUG_FUNCTION_EXIT("add_entry_to_file_list", -1);
        return -1;
                           ...
                           unsigned long *original_size_out)
{
    ...
    if (create_temp_filename(temp_file, sizeof(temp_file), output_file,
                             TEMP_FILE_SUFFIX_COMPRESS) != FILE_OP_SUCCESS)
    {
        DEBUG_FUNCTION_EXIT("process_file", 0);
        return 0;
    }
    ...
    {
        printf("\n");
        print_processing_summary("Process (Compress + Encrypt)", input_file,
                                 output_file, original_input_size, final_size);
                           ...
                           unsigned long *original_size_out)
{
    ...
    if (create_temp_filename(temp_file, sizeof(temp_file), output_file,
                             TEMP_FILE_SUFFIX_DECRYPT) != FILE_OP_SUCCESS)
    {
        DEBUG_FUNCTION_EXIT("extract_file", 0);
        return 0;
    }
    ...
    {
        printf("\n");
        print_processing_summary("Extract (Decrypt+Decompress)", input_file,
                                 output_file, original_input_size, final_size);
