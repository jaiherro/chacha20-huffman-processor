                  ...
                  const char *password, int quiet)
{
    ...
    for (int i = 0; i < num_files; i++)
    {
        ...
        snprintf(output_path, sizeof(output_path), "%s/%s.secure", output_dir,
                 filename_only);
        ...
        DEBUG_INFO("Processing individual file: '%s' -> '%s'", input_files[i],
                   output_path);
        processed_size = process_file(input_files[i], output_path, password,
                                      quiet, &original_size);
    ...
    if (!quiet)
    {
        ...
        {
            print_operation_result(1,
                                   "Batch processing (some operations failed)");
        }
    }
