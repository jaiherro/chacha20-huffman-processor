...
int
batch_process (char *input_files[], int num_files, const char *output_dir,
               const char *password, int quiet)
{
    ...
    DEBUG_INFO ("Starting batch processing - %d files to directory: '%s'",
    ...
    for (int i = 0; i < num_files; i++)
        {
            DEBUG_TRACE ("Processing file %d/%d: '%s'", i + 1, num_files,
                         input_files[i]);
                    ...
                    if (!quiet)
                        {
                            fprintf (stderr,
                                     ...
                                     "or cannot be read. Skipping.\n",
                                     input_files[i]);
                        }
                    DEBUG_WARN ("Skipping non-existent file: '%s'",
                                input_files[i]);
                    failure_count++;
                    continue;
                }
            ...
            snprintf (output_path, sizeof (output_path), "%s/%s.secure",
            ...
            DEBUG_INFO ("Processing individual file: '%s' -> '%s'",
                        input_files[i], output_path);
            processed_size = process_file (input_files[i], output_path,
                                           password, quiet, &original_size);

            if (processed_size > 0)
                {
                    success_count++;
                    DEBUG_INFO ("File processed successfully - original: %lu "
                                "bytes, processed: %lu bytes",
                    ...
                    if (add_entry_to_file_list (input_files[i], output_path,
                                                original_size, processed_size,
                                                quiet)
                        != 0)
                        {
                ...
                {
                    failure_count++;
                    DEBUG_ERROR ("Failed to process file: '%s'",
                                 input_files[i]);
                    if (!quiet)
                        {
            ...
            if (failure_count == 0)
            ...
            else
                {
                    print_operation_result (
                        1, "Batch processing (some operations failed)");
                }
        }

    DEBUG_INFO (
        "Batch processing completed - %d successful, %d failed out of %d total",
