           ...
           0)
    {
        for (size_t i = 0; i < bytes_read; i++)
        {
            ...
            for (int j = 0; j < ctx->codes[symbol].code_len; j++)
            {
                ...
                {
                    if (fwrite(output_buffer, 1, output_pos, output) !=
                        output_pos)
                    {
                        fclose(input);
                        fclose(output);
        ...
        while (decoded < original_size && input_pos < bytes_read)
        {
            ...
            if (input_pos < bytes_read)
            {
                ...
                if (!current->left && !current->right)
                {
                     ...
                     * bytes decoded) */
                    if (!quiet &&
                        (decoded % 1024 == 0 || decoded == original_size))
                    {
                        print_progress_bar(decoded, original_size,
                                           PROGRESS_WIDTH);
                    }
                    ...
                    {
                        if (fwrite(output_buffer, 1, output_count, output) !=
                            output_count)
                        {
                            free_tree(root);
                            fclose(input);
