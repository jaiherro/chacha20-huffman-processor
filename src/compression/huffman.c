...
unsigned long
huffman_worst_case_size (unsigned long input_len)
{
    return input_len + MAX_SYMBOLS * 10 + sizeof (unsigned long) + 256;
}

static void
pq_insert (priority_queue *pq, huffman_node *node)
{
    int i = pq->count;
    pq->nodes[i] = node;
...
static huffman_node *
pq_extract_min (priority_queue *pq)
{
    if (pq->count == 0)
        return NULL;
...
static huffman_node *
build_tree (unsigned long freq[MAX_SYMBOLS])
{
    priority_queue pq = { 0 };
...
static void
generate_codes (huffman_node *root, huffman_code codes[MAX_SYMBOLS],
                unsigned char *code, int len)
{
    if (!root)
        return;
...
static void
write_bit (unsigned char *out, unsigned long *pos, int *bit, int val)
{
    if (*bit == 0)
...
static int
read_bit (const unsigned char *in, unsigned long *pos, int *bit)
{
...
static int
write_tree (huffman_node *root, unsigned char *out, unsigned long *pos,
            int *bit)
{
    if (!root)
        return 0;
...
static huffman_node *
read_tree (const unsigned char *in, unsigned long *pos, int *bit)
{
    huffman_node *node = malloc (sizeof (huffman_node));
    if (!node)
        return NULL;
...
int
huffman_stream_init (huffman_stream_context *ctx)
{
    if (!ctx)
        return -1;
...
int
huffman_stream_count_frequencies (huffman_stream_context *ctx,
...
int
huffman_stream_prepare_encoding (huffman_stream_context *ctx)
{
    if (!ctx || ctx->pass != 1)
        return -1;
...
int
huffman_stream_compress_file (huffman_stream_context *ctx,
...
int
huffman_stream_decompress_file (const char *input_file, const char *output_file,
                                int quiet)
{
...
void
huffman_stream_cleanup (huffman_stream_context *ctx)
{
    if (!ctx)
        return;
...
int
huffman_compress_file (const char *input_file, const char *output_file,
                       int quiet)
{
