...
print_usage (const char *program_name)
{
    ...
    printf (
        "  -p <input> <output>    Process a file (compress then encrypt)\n");
    printf (
        "  -u <input> <output>    Extract a file (decrypt then decompress)\n");
    printf (
        "  -l                     List processed files (from file_list.dat)\n");
    printf ("  -f <pattern>           Find files matching pattern in list\n");
    printf ("  -b <outdir> <files..>  Batch process (compress + encrypt) "
            "multiple files\n");
    printf ("  -h, --help             Show this help information\n\n");
    printf ("OPTIONS:\n");
    printf ("  -q                     Quiet mode (minimal output)\n");
    printf ("  --debug                Enable debug mode (verbose diagnostic "
            "output)\n\n");
    ...
    printf ("NOTE: Operations requiring encryption/decryption will prompt for "
            "a password.\n");
}

void
print_progress_bar (unsigned long current, unsigned long total,
                    unsigned long width)
{
...
void
print_operation_result (int result, const char *operation)
{
...
void
print_processing_summary (const char *operation, const char *input_file,
                          const char *output_file, unsigned long input_size,
...
void
print_section_header (const char *title)
{
    printf ("\n=== %s ===\n", title);
}
