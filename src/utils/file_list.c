    ...
    for (file_entry_t *current = list->head; current; current = current->next)
        {
            ...
            if (fwrite(&input_filename_len, sizeof(unsigned long), 1, file) != 1
                || fwrite(current->input_filename, 1, input_filename_len, file)
                       != input_filename_len
                || fwrite(&output_filename_len, sizeof(unsigned long), 1, file)
                       != 1
    ...
    for (unsigned long i = 0; i < count_from_file; i++)
        {
            ...
            if (fread(entry->input_filename, 1, input_filename_len, file)
                ...
                || fread(&entry->sequence_num, sizeof(unsigned long), 1, file)
                       != 1
                || fread(&entry->original_size, sizeof(unsigned long), 1, file)
                       != 1
                || fread(&entry->processed_size, sizeof(unsigned long), 1, file)
                       != 1)
                {
                    free(entry);
                    fclose(file);
