...
int file_list_save(file_list_t *list, const char *filename)
{
    ...
    for (file_entry_t *current = list->head; current; current = current->next)
    {
        ...
        if (fwrite(&input_filename_len, sizeof(unsigned long), 1, file) != 1 ||
            ...
            fwrite(current->output_filename, 1, output_filename_len, file) !=
                output_filename_len ||
            fwrite(&current->sequence_num, sizeof(unsigned long), 1, file) !=
                1 ||
            fwrite(&current->original_size, sizeof(unsigned long), 1, file) !=
                1 ||
            fwrite(&current->processed_size, sizeof(unsigned long), 1, file) !=
                1)
        {
            fclose(file);
            return -1;
